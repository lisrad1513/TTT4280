...
def finnd():
    ...
    percent = 10 # Litt usikker på hvorfor dette ikke er 0.1 :(
    C = np.sqrt(3*(musr+mua)*mua)
    d = (np.log(percent)) / (2*C)

    return d

d = finnd()

print(f"Reflektansdybde for rødt lys: {d[0] * 1e3:.2} mm")
print(f"Reflektansdybde for grønt lys: {d[1] * 1e3:.2} mm")
print(f"Reflektansdybde for blått lys: {d[2] * 1e3:.2} mm")