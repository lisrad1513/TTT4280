...
def finnT(bvf=0.01, oxy=0.8, depth = 1):
...
def finnKontrast(T_hoyt, T_lavt):
    K = (np.abs(T_hoyt - T_lavt) / T_lavt)
    return K


b_blodaare = 300E-6 #m (300 µm)

T_100 = finnT(1, 0.8, b_blodaare)
T_001 = finnT(0.01, 0.8, b_blodaare)

print(f"Transmittans for 100% blodvolum fraksjon: {', '.join(f'{T*100:.4}%' for T in T_100)}")
print(f"Transmittans for 1% blodvolum fraksjon: {', '.join(f'{T*100:.4}%' for T in T_001)}")

K = finnKontrast(T_100, T_001)

print(f"Kontrast mellom 100% og 1% blodvolum fraksjon: {', '.join(f'{k:.4}' for k in K)}")